...
The plugin mounts its workspace-instruction injection pipeline at HOST scope:
an unscoped `ctx.on('agent/pre-step')` listener receives every agent's
pre-step regardless of preset, so instruction-scan works in **all modes**
(standard / liangshen / code / …) — no preset editing or special preset
selection needed. Installation is fully automatic: the first activation seeds
`~/.dsh/dsh-instruction-scan.json` (parents mode by default).
...
2. Restart DSH (the host half mounts the pipeline on activation).
3. Use ANY preset for new sessions. Workspace instructions are injected per
   the configured four-layer scan.
4. Toggle layers from Settings → Plugins → AGENTS扫描; changes persist to
   `~/.dsh/dsh-instruction-scan.json` and the pipeline reads disk config on
   every session start.

> **No duplicates**: the pipeline removes every pending `agent-instructions`
> inbox message (including those injected by the built-in
> `dsh-agent-instructions` row in each preset) before injecting its own
> synthesized result, so the built-in discovery is fully replaced.
